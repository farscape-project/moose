...
VectorDivDirichletBC::computeOffDiagJacobian(const unsigned int jvar_num)
{
  ...
  for (_j = 0; _j < phi_size; _j++)
  {
    if (_var.number() == jvar_num)
      _local_ke(_current_side, _j) = _j == _current_side;
    else
      _local_ke(_current_side, _j) = 0;
  }

  setTaggedLocalMatrix();
}
...
VectorDivDirichletBC::computeQpResidual()
{
  ...
  return ((_u[_qp] - u_exact) * (_normals[_qp])) * ((_test[_i][_qp]) * (_normals[_qp])) / abs((_test[_i][_qp]) * (_normals[_qp]));
}
